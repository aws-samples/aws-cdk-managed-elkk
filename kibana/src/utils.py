...
def generate_url(event: dict) -> Tuple[str, Optional[str]]:
    ...
    if params:
        url = f'{SERVER_URL}{path}?{params}'
    else:
        url = f'{SERVER_URL}{path}'
    return url, params


def exception_response(e: requests.RequestException,
...
def redirect_to_object(data: bytes, event: dict, content_type: str):
    ...
    response = {
        ...
        'body': None,
        'headers': {
            'Location': f'{CF_BUCKET_DOMAIN}{event["path"]}',
...
def valid_request():
    return all([CF_BUCKET_DOMAIN, SERVER_URL, CACHE_BUCKET])
...
def send_to_es(url: str,
               body: dict,
               headers: dict,
               request_func: callable) -> Tuple[Union[bytes, str], str]:
