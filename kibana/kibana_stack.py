...
class KibanaStack(core.Stack):
    def __init__(
        ...
        **kwargs,
        ...
        # network load balancer for outside vpc access
        nlb = elb.NetworkLoadBalancer(
            self, "nlb", vpc=constants["vpc"], internet_facing=True
        )
        core.Tags.of(nlb).add("project", constants["PROJECT_TAG"])

        # create the vpc link to the nlb
        link = apigw.VpcLink(self, "vpc_link", targets=[nlb])
        core.Tags.of(link).add("project", constants["PROJECT_TAG"])

        # create rest api
        kibana_api = apigw.RestApi(self, "kibana_api")
        # tag the api gateway
        core.Tags.of(kibana_api).add("project", constants["PROJECT_TAG"])

        kibana_api.root.add_method(
            "ANY", authorization_type=apigw.AuthorizationType.NONE
        )

        integration = apigw.HttpIntegration(
            url=constants["aes_endpoint"],
            http_method="ANY",
            options=apigw.IntegrationOptions(
                connection_type=apigw.ConnectionType.VPC_LINK, vpc_link=link
            ),
            proxy=False,
        )

        self.output_props = {}
        self.output_props["aes_endpoint_api"] = kibana_api.url
        self.output_props["kibana_endpoint_api"] = f"{kibana_api.url}/_plugin/kibana"
